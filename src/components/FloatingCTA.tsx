...
export default function FloatingCTA({
  ...
  const callHref = useMemo(() => {
    ...
    // We don’t add UTM to tel: but we can to a fallback web page link if desired later.
    return tel;
  }, [resolvedPhone]);

  const waHref = useMemo(() => {
    // New WA deeplink format (works on web/phone): https://wa.me/<number>?text=<encoded>
  ...
  return (
    <div className="fab z-fab">
      ...
      >
        {/* WA Icon (inline SVG, no external deps) */}
      ...
      <a href={callHref} aria-label={labels?.call ?? "Call EverCare"} className="btn shadow-soft">
        {/* Phone Icon */}
        <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
          <path
            d="M6.62 10.79a15.05 15.05 0 0 0 6.59 6.59l2.2-2.2a1 1 0 0 1 1.02-.24c1.12.37 2.33.57 3.57.57a1 1 0 0 1 1 1V21a1 1 0 0 1-1 1C10.07 22 2 13.93 2 3a1 1 0 0 1 1-1h3.5a1 1 0 0 1 1 1c0 1.24.2 2.45.57 3.57a1 1 0 0 1-.24 1.02l-2.2 2.2z"
            fill="currentColor"
          />
        </svg>
        <span>{labels?.call ?? "Call"}</span>
      </a>
    </div>
  );
}
