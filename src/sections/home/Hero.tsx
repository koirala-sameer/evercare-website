...
export default function Hero() {
  ...
  return (
    ...
    >
      {/* Gradient Base */}
      <div className="absolute inset-0 -z-20 bg-gradient-to-br from-[#E8F7F5] via-white to-[#FFF3F3]" />

      {/* Abstract Soft Glow */}
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_30%_30%,rgba(14,147,132,0.15),transparent_60%),radial-gradient(circle_at_70%_70%,rgba(245,138,140,0.15),transparent_60%)] animate-pulse-slow" />

      {/* Hands Overlay */}
      <div className="absolute inset-0 -z-10">
        <img
          src="/images/hands-care-bw.jpg"
      ...
      >
        ...
        >
          <Link to="/services" aria-label="Go to Services" className="flex-1">
            <button
              className={`w-full group relative overflow-hidden rounded-lg bg-gradient-to-br from-[#0E9384] to-[#0A7568] px-6 sm:px-8 py-3 sm:py-4 text-white font-semibold shadow-2xl transition-all duration-300 hover:shadow-2xl hover:shadow-[#0E9384]/30 ${
                touchEnabled
                  ? "mobile-tap-feedback"
                  : "hover:scale-[1.02]"
              } active:scale-[0.98]`}
            >
              <span className="relative z-10 tracking-tight text-sm sm:text-base">
                Get Started
              </span>
          ...
          <Link to="/services" aria-label="View Services" className="flex-1">
            <button
              className={`w-full group relative rounded-lg border-2 border-[#0E9384] bg-transparent px-6 sm:px-8 py-3 sm:py-4 text-[#0E9384] font-semibold shadow-lg overflow-hidden transition-all duration-300 ${
                touchEnabled
                  ? "mobile-tap-feedback"
                  : "hover:bg-[#0E9384] hover:text-white hover:shadow-xl hover:scale-[1.02]"
              } active:scale-[0.98]`}
            >
              <span className="relative z-10 tracking-tight text-sm sm:text-base">
                Explore Services
          ...
          <div className="flex items-center gap-2">
            <div className="flex -space-x-2">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-5 w-5 sm:h-6 sm:w-6 rounded-full bg-gradient-to-br from-brand-teal to-[#f58a8c] border-2 border-white"
                ></div>
              ))}
